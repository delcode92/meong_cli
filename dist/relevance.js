 ...
 */
async function getRelevantHistory(currentMessage, history, maxHistory, api) {
    ...
    try {
        ...
        const historyEmbeddings = await Promise.all(conversationPairs.map(pair => api.getEmbedding(pair.user.content)));
        // Calculate similarity scores
        const similarities = historyEmbeddings.map(histEmbedding => cosineSimilarity(currentEmbedding, histEmbedding));
        ...
        const relevantHistory = topPairs.flatMap(pair => [pair.user, pair.assistant]);
