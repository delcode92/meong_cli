...
class CLI {
    ...
    async startChat(streaming = true) {
        (0, header_1.displayHeader)();
        (0, header_1.displayWelcomeMessage)();
        const conversation = this.history.getCurrentConversation();
        if (conversation.length > 0) {
            console.log(chalk_1.default.hex('#ffaa00')('📚 Continuing previous conversation...\n'));
        }
        while (true) {
            const input = readlineSync.question(chalk_1.default.green('You: '));
            if (input.toLowerCase() === 'exit') {
                console.log(chalk_1.default.hex('#ff6600')('👋 Goodbye from MEONG CLI! See you next time! 🐱'));
                break;
            }
            ...
            try {
                console.log(chalk_1.default.hex('#00ffff')('🤖 SmolLM: '), { newline: false });
                if (streaming) {
    ...
    clearConversation() {
        ...
        console.log(chalk_1.default.hex('#00ff00')('🧹 Conversation cleared! Ready for a fresh start! ✨'));
    }
    newConversation() {
        this.history.newConversation();
        console.log(chalk_1.default.hex('#ff00ff')('🆕 Started new conversation! Let\'s chat! 💬'));
    }
    listConversations() {
